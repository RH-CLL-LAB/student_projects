...
cox_df2$TT2L_group <- ifelse(cox_df2$TT2L_group %in% c("POD12", "POD24"),
                                  "Early",
                                  "Late")
#Follow-up time####
cox_df2 <- cox_df2 %>% 
...
cox_df <- cox_df2 %>% 
  mutate(
    ...
    Age = age_at_2L,
    TT2L=factor(cox_df2$TT2L_group, levels = c("Late", "Early")))
...
#Sensitivity with the 6 patients with date_pato after date_treatment_2nd_line
pato_after <- fread("pato_6pt.txt")
cox_df3_uden_6pato <- cox_df3 %>% 
  anti_join(pato_after %>% select(patientid),
            by= "patientid")

Supfig3A <- KM_plot(survfit(Surv(surv_time_relapse/365.25, dead) ~ CD20, 
                            data = cox_df3_uden_6pato),
                    breaks = 2,
                    xlim = c(0,10),
                    title = 'A- All subtypes',
                    labs = c('CD20 Positive', 'CD20 Negative'),
                    pval = TRUE,
                    xlab= "Time (years from second-line therapy)",
                    ylab= "Overall survival probability")
...
#different subtypes
Supfig3B <- KM_plot(survfit(Surv(surv_time_relapse/365.25, dead) ~ CD20, 
                            data = cox_df3_uden_6pato %>% filter(subtype == "DLBCL")),
                    breaks = 2,
                    xlim = c(0,10),
                    title = 'B- DLBCL',
                    labs = c('CD20 Positive', 'CD20 Negative'),
...
Supfig3C <- KM_plot(survfit(Surv(surv_time_relapse/365.25, dead) ~ CD20, 
                            data = cox_df3_uden_6pato %>% filter(subtype == "FL")),
                    breaks = 2,
                    xlim = c(0,10),
                    title = 'C- FL',
                    labs = c('CD20 Positive', 'CD20 Negative'),
...
Supfig3D <- KM_plot(survfit(Surv(surv_time_relapse/365.25, dead) ~ CD20, 
                            data = cox_df3_uden_6pato %>% filter(subtype == "MCL")),
                    breaks = 2,
                    xlim = c(0,10),
                    title = 'D- MCL',
                    labs = c('CD20 Positive', ' CD20 Negative'),
...
library("gridExtra")
grid.arrange(Supfig3A$plot, Supfig3B$plot, Supfig3C$plot, Supfig3D$plot, nrow = 2, ncol=2)

#ggsave('/ngc/projects2/dalyca_r/sangho_r/figures/Suppfigure_6_date_pato.png',
 #      arrange_ggsurvplots(list(Supfig3A, Supfig3C, Supfig3B, Supfig3D), nrow = 2, ncol = 4),
  #     dpi=300,
   #    height = 10,
    #   width = 18) 
#ggsave('/ngc/projects2/dalyca_r/sangho_r/figures/Suppfigure_6_date_pato.pdf',
 #      arrange_ggsurvplots(list(Supfig3A, Supfig3C, Supfig3B, Supfig3D), nrow = 2, ncol = 4),
  #     dpi=300,
   #    height = 10,
    #   width = 18) 
